...
$A_i=i+\lambda,\quad B_i=d-i-\lambda$
...
$c_i^{[0]}=c_i,\quad c_i^{[r+1]}=ic_i^{[r]}+(d+r-i+1)c_{i-1}^{[r]}$

$x^qF(x)=\sum_{i=1}^{d+q}c_i^{[q]}\Phi_{i,d+q}(x),\quad c_i^{[r]}=0\ (i\notin[1,d+r])$
...
$\sum_{t=1}^{x}F(t)\binom{x-t+s}{s}\Longrightarrow
\nxt{c}_i=c_i,\quad \nxt{d}=d+s+1$

$\sum_{t=1}^{x-1}F(t)\binom{x-t+s-1}{s}\Longrightarrow
\nxt{c}_i=c_{i-1},\quad \nxt{d}=d+s+1$

$F(M)=\sum_{i=1}^{\min(d,M)}c_i\binom{M-i+d-1}{d-1}$
